...
import usf.gwt.timeline.event.GroupClickEvent;
import usf.gwt.timeline.event.GroupClickEvent.GroupClickHandler;
import usf.gwt.timeline.event.GroupClickEvent.HasGroupClickHandler;
import usf.gwt.timeline.event.ItemSelectionChangeEvent;
import usf.gwt.timeline.event.ItemSelectionChangeEvent.HasItemSelectionChangeHandler;
...
public class TimeLine extends Widget implements HasItemSelectionChangeHandler,
	...
	protected final static native void addGroup(final TimeLine t, final String groupName, final boolean c)/*-{
		var o = t.@usf.gwt.timeline.TimeLine::timeline;
		o.addItem({group : groupName});
		if(!c) return;
		var e = o.dom.groups.labels[o.dom.groups.labels.length - 1];
		$wnd.$(e).click(function(e) {
			t.@usf.gwt.timeline.TimeLine::fireGroupClick(Ljava/lang/String;)(e.target.textContent);
		});
	...
	protected final static native void attachClickEvent(final TimeLine t) /*-{
		...
		$wnd.links.events.addListener(o,'select', function() {
			if (o.getSelection()[0]) {
				...
				e.clickable && t.@usf.gwt.timeline.TimeLine::fireItemSelectEvent(Lcom/google/gwt/core/client/JavaScriptObject;)(e);
				o.setSelection({});
			}
		});
	}-*/;
